...
> 反思：
> My project used Android for deployment, but through that process, I realized that the choice of platform—whether it's a <b>native app</b>, a <b>web portal</b>, a <b>WeChat Mini Program</b> (like we use in China), or even models published via <b>emerging AI marketplaces</b> (like Baidu Smart Agent or the new model platforms) — is ultimately a strategic decision driven by the <b>user's context</b> and the <b>task requirements</b>.
...
I haven’t fixed my long-term career plan yet, because I believe suitability must be tested through practice. In the short term, I want to broaden my horizons and gain real experience. My philosophy is clear: I want to build practical technologies that people can use in daily life. I hope to make technology accessible and turn code into tangible products that people can interact with.

### 个人信息

> My Career Development Views (Organized Version) 
> Regarding my career development plan, direction and time scale are two key factors—choosing the right field and deciding how long to stick to it.
> First, on career directions, I have three possible paths in mind. The first is working in the industry as a developer, such as a full-stack, front-end, back-end, data developer, or AI engineer. The second is pursuing academic research, meaning continuing to a PhD after my master’s (either industry-collaborated PhD, institute-based PhD, or university PhD). The third is a mixed path: working first then doing a PhD later, or going into industry after getting a PhD. Ultimately, my career will lie in either industry, academia, or a cross of both. 
> Second, in terms of time scale, my master’s program is a crucial period. During this time, I need to choose courses based on my target direction and actively seize opportunities—internships, teaching assistant roles, etc. But since I can’t confirm my direction quickly in 2-3 years, my short-term plan (next 6-12 months) is to find my true direction through practice. I believe assumptions about a field (like judging a course only by its title) are unreliable; real experience is key. So I’ll take real tasks, get to know how academia and industry work in practice to make informed choices.
> For long-term plans, I don’t have a fixed one yet—I’m still exploring. But my core value/philosophy has always been: I want to create practical things that people can use in daily life. I hope to make technology accessible and turn code into tangible products that people can interact with.

---

## 完整示范回答 (Complete Sample Answers)

### 🔍 技术与项目经验 (Technical & Project Competence)

<b>(Q1, Q2, Q3 已在前面回答，这里不再重复)</b>

### 🧠 研究兴趣与知识广度 (Research Interest & Knowledge Breadth)

#### Q1: How do you see your <b>Edge AI experience connecting</b> to our Mixed Reality research?

> <b>A1:</b> I see a direct and vital connection. Mixed Reality applications—especially those focusing on natural Human-Computer Interaction—are fundamentally reliant on <b>low-latency, on-device perception</b>. My experience is precisely in achieving this: optimizing models for speed, managing resource constraints, and ensuring functional, low-latency deployment. In MR, we need instantaneous processing for tasks like <b>hand tracking, gaze estimation, and environment understanding</b>. My skills can be immediately applied to optimizing the <b>perception backbone</b> that drives your MR systems.
...
### 🚀 假设情景与行为问题 (Hypothetical & Behavioral)

#### Q10: Imagine you have a new AR project, but the budget is very limited. How would you prioritize the technical scope?

> <b>A10:</b> With limited resources, the priority must be the <b>Minimum Viable Product (MVP)</b>. I would first focus on the <b>core functionality</b>—for MR, that means robust, low-latency tracking and interaction (e.g., the hand gesture recognition). I would use open-source libraries or foundational tools (like MediaPipe) where possible, and <b>defer</b> complex, resource-intensive features (like high-fidelity environment reconstruction or advanced rendering) until more funding is secured. <b>Functionality over polish</b> would be the guiding principle.
...
> - `item_tfms=[Resize(224, method='squish')]`: resize each image to 224×224
...
> Next, I created a `DataBlock` to define the data pipeline. 输入是图片（`ImageBlock`），输出是类别标签（`CategoryBlock`），通过 `get_items=get_image_files` 获取所有图片，使用 `parent_label` 从父文件夹生成标签。
 我用 `RandomSplitter(valid_pct=0.2)` 随机划分训练集和验证集，占比 80/20。每个样本通过 `item_tfms=[Resize(224)]` 统一调整大小，batch 使用 `batch_tfms=custom_transforms` 应用增强。
 Finally, `dataloaders(path, bs=32)` 生成 DataLoader，每 batch 32 张图，`show_batch(max_n=15)` 可以快速可视化样本。”
...
> Batch size adjust
> After preparing the dataset and DataLoaders, I created a vision learner using FastAI’s `vision_learner` with a ResNet101 backbone. I chose it because it strikes a good balance between accuracy and model complexity, making it highly suitable for my animal - recognition task.我选择 ResNet101 是因为它在准确率和模型复杂度之间有良好的平衡，非常适合我的动物分类任务。
 I set accuracy as the main metric and added `CSVLogger` to record the training process for later analysis.”
> “Before training, I used FastAI’s `lr_find()` to search for the optimal learning rate for stable and fast convergence.(我选择 loss 曲线快速下降的点作为最佳学习率I choose the point where the loss curve drops rapidly as the optimal learning rate) 
 面试官可能会问，如果学习率太大或太小会发生什么——太大会导致梯度爆炸，太小则收敛缓慢。”If the learning rate is excessively large, it may lead to gradient explosion. Conversely, if the learning rate is too small, the convergence speed might be extremely slow.
> “Then I used `fine_tune()` for transfer learning, training the pretrained ResNet101 on my dataset for 15 epochs with  2 callbacks:
> - `EarlyStoppingCallback` stops training if validation loss doesn’t improve for 4 epochs, preventing overfitting.`EarlyStopping Callback` 监控验证集 loss，如果连续 4 个 epoch 没有下降就提前停止训练，避免过拟合。
> - `SaveModelCallback` saves the model with the lowest validation loss to ensure the best model is reserved.”保存验证集 loss 最小的模型，保证最终得到最优模型，而不是最后一个 epoch 的模型。”
...
- 遇到过什么问题？怎么解决？
- 例如：训练慢、内存不够、部署到 mobile 的问题

## Android 

### ✅ 模型怎么从训练端 → 部署到 Android

#### 你训练好的模型如何部署到移动端？

#### 用的是 ONNX / TensorFlow Lite / PyTorch Mobile？
...
Crucially, I decoupled the inference logic from the main UI thread. I used <b>Kotlin Coroutines</b> with the `Dispatchers.IO` context to perform the heavy lifting—specifically the model loading and the `recognizeAnimal` function—asynchronously. This ensures that the inference process never blocks the UI, which is a critical requirement for maintaining  a smooth UI flow。 <i>Translation:</i> 至关重要的是，我将推理逻辑与主 UI 线程解耦。我使用了 <b>Kotlin 协程</b> 和 `Dispatchers.IO` 上下文来异步执行繁重的任务——具体来说是模型加载和 `recognizeAnimal` 函数。这确保了推理过程绝不会阻塞 UI，这对于维持应用界面流畅是一个关键要求。
...
Yes, performance monitoring is embedded in my inference loop. I used `System.nanoTime()` to capture precise timestamps before and after the `model.run()` call to calculate the exact inference duration in milliseconds. <i>Translation:</i> 是的，性能监控嵌入在我的推理循环中。我使用 `System.nanoTime()` 在 `model.run()` 调用前后捕捉精确的时间戳，以计算毫秒级的确切推理时长。

### 端侧 AI 限制

### Data process

I implemented a strict preprocessing pipeline to match the model's training requirements. The raw `Bitmap` is first scaled to the target resolution of 224x224. Then, I manually convert the pixel data into a `FloatArray`, applying standard normalization with a specific mean and standard deviation (Mean: 0.485, 0.456, 0.406; Std: 0.229, 0.224, 0.225). <i>Translation:</i> 我实现了一个严格的预处理管线来匹配模型的训练要求。原始的 `Bitmap` 首先被缩放到 224x224 的目标分辨率。然后，我手动将像素数据转换为 `FloatArray`，并应用特定的均值和标准差（Mean: 0.485... Std: 0.229...）进行标准化

Finally, this float array is wrapped into a `FloatBuffer` and converted into an `OnnxTensor` with a shape of `[1, 3, 224, 224]`. This tensor serves as the input for the `ortSession.run()` method. This manual pipeline gives me full control over memory allocation and allows for future optimization using native C++ buffers if needed. <i>Translation:</i> 最后，这个浮点数组被封装进 `FloatBuffer` 并转换为形状为 `[1, 3, 224, 224]` 的 `OnnxTensor`。这个 Tensor 被用作 `ortSession.run()` 方法的输入。这种手动管线让我能完全控制内存分配，并且在未来需要时可以使用原生 C++ 缓冲区进行优化。

### ☁️ Part 4: The Hybrid Strategy (Edge + Cloud) (混合架构策略)

<b>Professor's Likely Question:</b><em>"You mentioned using Gemini API as well. How does your system decide when to use the local model versus the cloud API?"(你提到了使用 Gemini API。你的系统如何决定何时使用本地模型，何时使用云端 API？)</em>

<b>Your Answer:</b>

"I designed a fallback mechanism to balance latency and intelligence. The system prioritizes the local ONNX model for immediate, low-latency feedback. However, if the local recognition fails (returns an 'Unrecognized' state) or if the user requests deeper contextual details, the system triggers the cloud path. <i>Translation:</i> 我设计了一个回退机制来平衡延迟和智能。系统优先使用本地 ONNX 模型以获得即时、低延迟的反馈。然而，如果本地识别失败（返回“未识别”状态）或者用户请求更深层的上下文细节，系统会触发云端路径。"

"In that case, I encode the image to Base64 and send it to the Gemini API asynchronously. This hybrid architecture allows the application to be 'Real-Time' for basic interaction, but 'Intelligent' for complex queries, which fits perfectly with the requirements of Mixed Reality assistants. <i>Translation:</i> 在这种情况下，我将图像编码为 Base64 并异步发送给 Gemini API。这种混合架构使得应用在基础交互上是“实时”的，但在处理复杂查询时是“智能”的，这非常符合混合现实助手的需求。"

I’d like to briefly present one of my main projects
...
In particular, I needed to make sure that the image preprocessing steps — such as resizing, normalization and channel ordering — were exactly aligned between FastAI during training and the Android ONNX Runtime execution.
